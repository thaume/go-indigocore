...
GO_LIST=$(GO_CMD) list
GO_BUILD=$(GO_CMD) build  -ldflags '-extldflags "-static"' -gcflags=-trimpath=$(GOPATH) -asmflags=-trimpath=$(GOPATH) -ldflags '-X main.version=$(VERSION) -X main.commit=$(GIT_COMMIT) -X github.com/stratumn/sdk/strat/cmd.DefaultGeneratorsRef=$(GENERATOR_VERSION)'
GO_TEST=$(GO_CMD) test
